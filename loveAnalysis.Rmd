...
ggsave("observed.png", width = 10, height = 4)
```
...
mdl <- jags.model("loveModel.bugs", jagsData, n.chains = nChains, n.adapt = nAdapt, inits = inits)
```
...
```{r}
# save(codaSamples, file = "loveModelSamples.save", compress = T)
...
ggsave("observed_predicted_individual.png", width=12, height=5)
```
...
ggsave("predicted.png", width = 10, height = 4)
```
...
ggsave("predicted_mean.png", width = 10, height = 4)
```
...
```{r}
dicSamples <- dic.samples(mdl, nIter, thin = thinning, "pD")
show(dicSamples)
```
...
fit <- lmer(score ~ posFactor*month + (1 + month|subject), data = loveData)
summary(fit)
```
